    ...
    for i in range(rows):
        for j in range(cols):
            ...
            if j < cols - 1:  # Horizontal truss
                ...
                indices = [3*current_node, 3*current_node+1, 3*current_node+2,
                           3*next_node, 3*next_node+1, 3*next_node+2]
                for ii in range(4):
                    for jj in range(4):
                        stiffness_matrix[indices[ii % 2], indices[jj % 2]] += k_local[ii % 2, jj % 2]
...
# Boundary Conditions: Fix all DOFs at two diagonal points
fixed_dofs = [0, 1, 2, 3 * (grid_size[0] * grid_size[1] - 1),
              3 * (grid_size[0] * grid_size[1] - 1) + 1, 3 * (grid_size[0] * grid_size[1] - 1) + 2]
