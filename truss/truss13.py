...
def generate_nodes(n, spacing=1.0, h=1.0):
    """
    Generates node coordinates for top and base grids.
    
    Parameters:
    - n: Number of nodes per side (n x n grid).
    - spacing: Distance between adjacent nodes in x and y directions.
    - h: Height difference between top and base grids.
    
    Returns:
    - node_coords: Numpy array of shape (2*n*n, 3) with (x, y, z) coordinates.
    """
    top_nodes = []
    base_nodes = []
    for i in range(n):
...
def get_truss_connections(n):
    """
    Defines truss connections for top and base grids, and vertical trusses.
...
def visualize_truss_3d(node_coords, connections, displacements, scale=1.0):
    ...
    ax.set_title("3D Truss Fabric Simulation")
    ax.set_xlabel("X")
    ax.set_ylabel("Y")
    ax.set_zlabel("Z")
...
def main():
    ...
    # Define boundary conditions (fix all DOFs of two diagonal top nodes: node 0 and node n*n -1)
    ...
    sample_nodes = [1, n, n*n -2, 2*n*n -1]
